...
let test_lwt_ipc =
  ...
  Bench.Test.(
      [
        create ~name:"lwt_ipc-mpst_dynamic" (let module M = MakeDyn(DynCheckMutex)(LwtMonad)(IPC)() in run M.runtest);
        create ~name:"lwt_ipc-mpst_static" (let module M = MakeStatic(LinLwtMonad)(IPC)() in run M.runtest);
        create ~name:"lwt_ipc-OCaml_ideal" (let module M = Make_IPC(LwtMonad)() in run M.runtest);
      ]
  )
...
let test_ev_ipc =
  ...
  Bench.Test.(
      [
        create ~name:"ev_ipc-mpst_dynamic" (let module M = MakeDyn(DynCheckMutex)(Direct)(IPC)() in run M.runtest);
        create ~name:"ev_ipc-mpst_static" (let module M = MakeStatic(LinDirect)(IPC)() in run M.runtest);
        create ~name:"ev_ipc-OCaml_ideal" (let module M = Make_IPC(Direct)() in run M.runtest);
      ]
  )
...
let test_lwt =
  ...
  Bench.Test.(
      [
        ...
        (* create_indexed ~name:"lwt(bstream)-OCaml_cps" ~args (let module M = BLwtCont(LwtBoundedStream)() in run M.runtest);
         * create_indexed ~name:"lwt(wake)-OCaml_cps" ~args (let module M = BLwtCont(LwtWait)() in run M.runtest); *)
...
let test_iteration =
  ...
  Bench.Test.(
      [
         ...
         * create ~name:"mpst-static/ev" @@ (let module M = MakeStatic(LinDirect)(Shmem)() in run M.runtest);
         * create ~name:"mpst-dynamic/ev" (let module M = MakeDyn(DynCheckMutex)(Direct)(Shmem)() in run M.runtest);
         ...
         * create ~name:"OCaml-ev/twochan" BEvent.runtest; *)
  ])
