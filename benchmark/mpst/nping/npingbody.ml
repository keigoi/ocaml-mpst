...
module MakeDyn (Med : MEDIUM) () : TEST = struct
  module Test = struct
    let rec gen_nping n t =
      if n = 0 then t
      else (a --> b) ping @@ (b --> a) pong @@ gen_nping (n - 1) t

    let nping n = fix (fun t -> gen_nping n t)

    (** * pre-allocated channels *)
    let sa_init, sb_init = (ref None, ref None)
...
module MakeStatic (Med : MEDIUM) () = (* : TEST *)
struct
  open Mpst_lin
  ...
  module Test = struct
    ...
    (** * pre-allocated channels *)
    let sa_stored, sb_stored = (ref None, ref None)
      ...
      fun [@inline] () ->
        ...
        let* (_ : all_empty), { __lin = sb } =
          Syntax.Internal._run
            ...
            @@ let%lin (`ping (_, #s)) =
                 receive s (fun [@inline] x -> x#role_A)
               in
    ...
    let client_step param =
      let payload = () in
      Core.Staged.stage (fun [@inline] () ->
          let sa = from_some !sa_stored in
          let* ( (_ : all_empty),
                 { __lin = `pong ({ Linocaml.data = () }, { __lin = sa }) } ) =
            Syntax.Internal._run
              (Syntax.Internal._modify (Syntax.lens_put' s sa)
              @@ let%lin #s =
                   send s (fun [@inline] x -> x#role_B#ping) payload
                 in
