...
let prefix = if IO.is_direct then "ev_" else "lwt_"
...
let test =
  [
     ...
     *)
    create ~name:(prefix ^ "dynamic")
      (let module M = MakeDyn (Shmem) () in
      run M.runtest);
    create
      ~name:(prefix ^ "dynamic_untyped")
      (let module M = MakeDyn (Untyped) () in
      run M.runtest);
    (create ~name:(prefix ^ "static")
    @@
    let module M = MakeStatic (Shmem) () in
    run M.runtest);
    (create ~name:(prefix ^ "static_untyped")
    @@
...
let test_ipc =
  if IO.is_direct then
    [
       ...
       *)
      create_indexed ~args ~name:"ipc_dynamic"
        (let module M = MakeDyn (IPC) () in
        M.runtest);
      create_indexed ~args ~name:"ipc_static"
        (let module M = MakeStatic (IPC) () in
        M.runtest);
    ]
  else []

let test_all = test @ test_ipc
let () = Core.Command.run @@ Core_bench.Bench.make_command test_all
