...
let create_resolver_fd ~fd ~peer_addr = { fd; peer_addr; query_id = 0 }
...
let create_listener_fd ~fd =
  {
    ...
    peer_addr =
      Lwt_unix.(
        ADDR_INET (Ipaddr_unix.to_inet_addr (Ipaddr.of_string_exn "0.0.0.0"), 0));
    query_id = 0;
  }
  ...
  Slabel
    {
      ...
      handler =
        {
          write =
            (fun ({ fd; peer_addr; _ } as r) query ->
              (* XXX move dstaddr to connection type??*)
              r.query_id <- query.Dns.Packet.id;
              let buf = Dns.Packet.marshal query in
              Cstruct.(
                Lwt_bytes.sendto fd buf.buffer buf.off buf.len [] peer_addr)
              >>= fun _ -> Lwt.return_unit);
          read =
            (fun ({ fd; _ } as r) bufopt ->
              ...
              (*fixme bufferiing*)
              Lwt.return
                (bufopt, Dns.Protocol.Server.parse (Cstruct.sub buf 0 len)));
        };
    }
  ...
  Slabel
    {
      ...
      handler =
        {
          write =
            (fun { fd; peer_addr; _ } resp ->
              let buf = Dns.Packet.marshal resp in
              Cstruct.(
                Lwt_bytes.sendto fd buf.buffer buf.off buf.len [] peer_addr)
              >>= fun _ -> Lwt.return_unit);
          read =
            (fun { fd; peer_addr; query_id } bufopt ->
              let buf = Cstruct.create 4096 in
              Cstruct.(Lwt_bytes.recvfrom fd buf.buffer buf.off buf.len [])
              >>= fun (len, dst_addr) ->
