...
let tCli ec =
  let ec = send (ec#compute) (Add, 20) in
  let ec = send (ec#compute) (Sub, 45) in
  let ec = send (ec#compute) (Mul, 10) in
  let ec = send (ec#result) () in
  let `answer(ans, ec) = Event.sync ec in
  close ec;
