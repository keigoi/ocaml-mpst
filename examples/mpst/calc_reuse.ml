...
let calcbody self =
  choice_at cli (to_srv compute_or_result)
    (cli, (cli --> srv) compute @@ self)
    (cli, (cli --> srv) result @@ (srv --> cli) answer @@ finish)

let calc' () = fix (fun t -> calcbody t)
...
let (_ : unit IO.io) =
  let g = gen @@ calc' () in
  let ec = get_ch cli g and es = get_ch srv g in
...
let compute_result_or_current =
  {
    disj_concat =
      (fun l r ->
        object
          method compute = l#compute
          method result = l#result
          method current = r#current
        end);
...
let calc2' () =
  fix (fun t ->
      choice_at cli
        (to_srv compute_result_or_current)
        (cli, calcbody t)
        (cli, (cli --> srv) current @@ (srv --> cli) answer @@ t))

let tSrv2' es =
  let rec loop acc es =
    let* label = receive es#role_Cli in
    match label with
    | (`compute _ | `result _) as v -> srvbody loop acc v
    | `current ((), es) ->
        let* es = send es#role_Cli#answer acc in
        loop acc es
  in
...
let () =
  ...
  @@ IO_list.iter Thread.join
       [ Thread.create tCli2 ec; Thread.create tSrv2' es ]
