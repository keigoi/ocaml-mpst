...
let test1 =
  ...
  with
    Mpst.UnguardedLoopSeq ->
    print_endline "  exception correctly occurred (test1)\n"

(* merging failure between "end" type and reception *)
let test2 =
  print_endline "test2: checking that a merging error between μt.t and input type detected correctly";
  try
    let _g =
      gen @@
      choice_at a (to_b left_or_right)
        (a, (a --> b) left @@
            fix (fun t -> (a --> b) msg @@ (* closed_at c *) t)) (* this "closed_at c" should be there, according to MPST theory. (!) *)
        (a, (a --> b) right @@
            (a --> c) msg @@ finish)
    in
