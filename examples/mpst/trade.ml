  ...
  let to_c_or_s =
    {
      disj_concat =
        (fun l r ->
          object
            method role_C = l#role_C
            method role_S = r#role_S
          end);
  ...
  let offer_or_result =
    {
      disj_concat =
        (fun l r ->
          object
            method offer = l#offer
            method result = r#result
          end);
...
let g =
  ...
  @@ fix (fun t ->
         choice_at b (to_c result_or_offer)
           (b, (b --> c) result @@ (b --> s) final @@ finish)
           (b, (b --> c) offer @@ (c --> b) counter @@ t))
...
let _ =
  ...
  @@ IO_list.iter Thread.join
       (List.map (fun f -> Thread.create f ()) [ seller 250; bob; carol ])
