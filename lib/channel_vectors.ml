...
module MakeSingle (Low : LOW_COMM) (DynLin : Dyn_lin.S) : sig
  ...
  val declare_inp :
    ('obj, 'var inp) method_ -> 'var Low.inp -> 's local -> 'obj local
  ...
  let declare_out role label out cont =
    ...
    in
    let merge_out_lin x y =
      DynLin.merge merge_out (Base.compose_method role label) x y
    in
    Mergeable.make ~value:ch ~cont ~mergefun:merge_out_lin ()

  let declare_inp role inp cont =
    let ch =
      (* <role_ri = wrap (receive ch) (λx.`lab(x,sj)) > *)
      DynLin.wrap role.make_obj @@ DynLin.declare inp
    in
    let merge_inp_lin x y = DynLin.merge Low.merge_inp role x y in
    Mergeable.make ~value:ch ~cont ~mergefun:merge_inp_lin ()

  let declare_close f =
    Mergeable.make ~value:(DynLin.declare f)
  ...
  let[@inline] receive (ch : 'var inp) =
    IO.bind (DynLin.use ch) (fun [@inline] ch -> Low.receive ch)

  let[@inline] close (ep : close) =
    IO.bind (DynLin.use ep) (fun [@inline] f -> f ())
end
[@@inline]
...
module MakeScatterGather (Local : LOW_COMM) (DynLin : Dyn_lin.S) : sig
  ...
  let declare_out_many role label out cont =
    ...
    in
    let merge_out_many_lin =
      DynLin.merge merge_out_many (Base.compose_method role label)
    in
    Mergeable.make ~value:ch ~cont ~mergefun:merge_out_many_lin ()
  ...
  let declare_inp_many role inp cont =
    ...
    Mergeable.make ~value:ch ~cont ~mergefun:merge_inp_many_lin ()
