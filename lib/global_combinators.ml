...
module Make (DynLin : Dyn_lin.S) (Lin : LIN) : sig
  ...
  let is_typed env from to_ =
    let from = int_of_idx from.role_index and to_ = int_of_idx to_.role_index in
    let from_info = Env.metainfo env from 1
    and to_info = Env.metainfo env to_ 1 in
    from_info.rm_kind = Env.EpLocal && to_info.rm_kind = Env.EpLocal

  (** * Communication combinator *)
  let ( --> ) ri rj label (g0 : _ global) : _ global =
   fun env ->
    let g0 = g0 env in
    ...
    let out, inp =
      if is_typed env ri rj then
        ...
        Low.create_untyped env ~from:(int_of_idx ri.role_index)
          ~to_:(int_of_idx rj.role_index) label sj'
    in
  ...
  let gather ri rj label (g0 : _ global) : _ global =
   fun env ->
    ...
    let outs, inpmany =
      if is_typed env ri rj then
        ...
        Low.create_untyped_inp_many env ~from:(int_of_idx ri.role_index)
          ~to_:(int_of_idx rj.role_index) label sj'
    in
  ...
  let scatter ri rj label (g0 : _ global) : _ global =
   fun env ->
    ...
    let out_many, inps =
      if is_typed env ri rj then
        ...
        Low.create_untyped_out_many env ~from:(int_of_idx ri.role_index)
          ~to_:(int_of_idx rj.role_index) label sj'
    in
    ...
    let si' = Seq.get ri.role_index g1 in
    let si =
      ScatterGather.declare_out_many rj.role_label label.obj out_many si'
    in
    let g2 = Seq.put ri.role_index g1 si in
    g2
  ...
  let choice_at r disj (r', g0left) (r'', g0right) env =
    let g0left, g0right = (g0left env, g0right env) in
    let epL, epR =
      (Seq.get r'.role_index g0left, Seq.get r''.role_index g0right)
    in
    let g1left, g1right =
      (Seq.put r'.role_index g0left munit, Seq.put r''.role_index g0right munit)
    in
    let g1 = Seq.seq_merge g1left g1right in
    let disj = DynLin.lift_disj disj in
    (* lift *)
    let ep = Mergeable.make_disj disj epL epR in
    let g2 = Seq.put r.role_index g1 ep in
    g2
  ...
  module Fix = struct
    ...
    let fix :
        type g.
        (g, ([ `cons of close_ one * 'a ] as 'a)) idxs ->
        (g global -> g global) ->
        g global =
     fun _idxs f env ->
      let rec body = lazy ((f (fun _ -> Seq.recvar body)) env) in
  ...
  let fix : type g. (g global -> g global) -> g global =
   fun f env ->
    let rec body = lazy ((f (fun _ -> Seq.recvar body)) env) in
    (* A "fail-fast" approach to detect unguarded loops.
     * Seq.partial_force tries to fully evaluate unguarded recursion variables
  ...
  let finish_seq env = Seq.repeat 0 (fun role -> make_close env role 0)
  let finish : ([ `cons of close_ one * 'a ] as 'a) global = finish_seq

  let finish_with_multirole :
      at:
        ( close_ one,
          close_ list,
          ([ `cons of close_ one * 'a ] as 'a),
          'g,
          _,
          _ )
        role ->
   ...
   fun ~at env ->
    ...
    let g' =
      Seq.put_list at.role_index seq
        (List.init count (fun i -> make_close env role i))
    in
    g'

  let closed_at :
        'g.
        (close_ one, close_ one, 'g, 'g, _, _) role -> 'g global -> 'g global =
   fun r g env ->
    let g = g env in
  ...
  let closed_list_at :
        'g.
        (close_ list, close_ list, 'g, 'g, _, _) role -> 'g global -> 'g global
      =
    closed_list_at_
  ...
  let ipc cnt = Env.EpDpipe (List.init cnt (fun _ -> Table.create ()))
  let untyped cnt = Env.EpUntyped (List.init cnt (fun _ -> Table.create ()))
  ...
  let gen_mult ps g =
    let ps =
      List.mapi
        (fun i cnt -> { Env.rm_index = i; rm_size = cnt; rm_kind = EpLocal })
        ps
    in
    gen_with_env { Env.metainfo = Table.create_from ps; default = local } g
  ...
  let mkparams_mult ps =
    {
      Env.metainfo =
        Table.create_from
          (List.mapi
             (fun rm_index (k, rm_size) -> rm_kind_of_kind ~rm_index ~rm_size k)
             ps);
      default = local;
    }
  ...
  let mkparams ps =
    {
      Env.metainfo =
        Table.create_from
          (List.mapi
             (fun rm_index k -> rm_kind_of_kind ~rm_index ~rm_size:1 k)
             ps);
      default = local;
    }
  ...
  let effective_length (_, s) = Seq.effective_length s
  ...
  type 'a ty = Ty__ of (unit -> 'a) | TyList__ of (unit -> 'a list)

  let get_ty_ r g = Ty__ (fun () -> Lin.mklin @@ get_ch r g)
  let get_ty r g = get_ty_ r (gen g)

  let get_ty_list_ r g =
    TyList__ (fun () -> List.map Lin.mklin @@ get_ch_list r g)

  let get_ty_list r g = get_ty_list_ r (gen g)
  let ( >: ) l _ = l
  let env (env, _) = env
