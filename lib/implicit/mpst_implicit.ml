...
module Make(X:sig type conn end)(E:EVENT) = struct
  type conn = X.conn
  ...
  let finish : ([`seq of (ConnTable.t -> close) * 'a] as 'a) seq =
  ...
  let choice_at : 'k 'ep 'ep_l 'ep_r 'g0_l 'g0_r 'g1 'g2.
    ...
    let g1left, g1right =
      put r'.lens g0left (val_ (fun _ -> ())),
      put r''.lens g0right (val_ (fun _ -> ())) in
  ...
  module MakeGlobal(X:LIN) = struct
    ...
    let make_recv rA lab epB =
      ...
      in
      val_ @@ make_mergeable_fun (fun x y -> E.choose [x;y]) rA.label ev
...
module IPC = struct
  ...
  include Mpst_base.Connection.Make(Raw)
  open Raw
