...
let[@inline] send idx sel v =
  {
    __m =
      (fun [@inline] lpre ->
        let s = lens_get idx lpre in
        IO.bind
          (send_raw (sel (unlin s)) { data = v })
          (fun [@inline] t -> IO.return (lens_put idx lpre (), { __lin = t })));
  }

let deleg_send idx1 sel idx2 =
  {
    __m =
      (fun lpre ->
...
let[@inline] receive idx sel =
  {
    __m =
      (fun [@inline] lpre ->
        let s = lens_get idx lpre in
        IO.bind
          (receive_raw (sel (unlin s)))
          (fun [@inline] var ->
            IO.return (lens_put idx lpre (), { __lin = var })));
  }

let[@inline] send_many idx sel f =
  {
    __m =
      (fun [@inline] lpre ->
        let s = lens_get idx lpre in
        IO.bind
          (send_many_raw (sel (unlin s)) (fun x -> { data = f x }))
          (fun [@inline] t -> IO.return (lens_put idx lpre (), { __lin = t })));
  }

let[@inline] receive_many idx sel =
  {
    __m =
      (fun [@inline] lpre ->
        let s = lens_get idx lpre in
        IO.bind
          (receive_many_raw (sel (unlin s)))
          (fun [@inline] var ->
            IO.return (lens_put idx lpre (), { __lin = var })));
  }

let[@inline] close idx =
  {
    __m =
      (fun [@inline] lpre ->
        let s = lens_get idx lpre in
        IO.bind
          (close_raw (unlin s))
          (fun [@inline] () -> IO.return (lens_put idx lpre (), { data = () })));
  }
  ...
  let close =
    {
      __m =
        (fun [@inline] lpre ->
          IO.bind
            (close_raw (unlin lpre))
            (fun [@inline] () -> IO.return ((), { data = () })));
    }
  ...
  let create_thread_lin (m : unit -> ('a lin, unit, unit data) monad) =
    {
      __m =
        (fun lpre ->
          let (_ : Thread.t) =
            Thread.create (fun x -> Syntax.Internal._run (m ()) x) lpre
          in
          IO.return ((), { data = () }));
    }
end
...
let linret f =
  { Linocaml.__m = (fun pre -> IO.return (pre, { Linocaml.__lin = f () })) }
...
let degen :
    ( ([ `cons of close one * 't ] as 't) tup lin,
      unit,
      unit data )
    Linocaml.monad =
...
let degen_ :
    (([ `cons of close one * 't ] as 't) tup lin, unit, 'pre, 'post) lens ->
    ('pre, 'post, unit data) Linocaml.monad =
 fun s ->
  {
    Linocaml.__m =
      (fun lpre -> IO.return (lens_put s lpre (), { Linocaml.data = () }));
  }
  ...
  let open Linocaml in
  {
    __m =
      (fun lpre ->
        ...
        let ep, g' = get_ch_ r g in
        IO.return
          (lens_put s lpre (), { __lin = ({ __lin = g' }, { __lin = ep }) }));
  }
