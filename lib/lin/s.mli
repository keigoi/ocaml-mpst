module type COMM_LIN = sig
  ...
  val send_many :
    ('a lin, unit, 'b, 'c) lens ->
    ((< .. > as 'a) -> ('d data, 'e) out_many) ->
    (int -> 'd) ->
    ('b, 'c, 'e lin) monad

  val receive_many :
    ('a lin, unit, 'b, 'c) lens -> ('a -> 'd inp_many) -> ('b, 'c, 'd lin) monad

  val close :
    (close lin, unit, 'pre, 'post) lens -> ('pre, 'post, unit data) monad
  ...
  module LinocamlStyle : sig
    ...
    val send :
      ((< .. > as 's) -> ('v data, 't) out) ->
      'v ->
      ('s lin, unit, 't lin) monad

    val deleg_send :
      ((< .. > as 's) -> ('t lin, 'u) out) ->
      ('s lin * 't lin, unit, 'u lin) monad

    val receive : ((< .. > as 's) -> 'var inp) -> ('s lin, unit, 'var lin) monad
    val close : (close lin, unit, unit data) monad

    val create_thread_lin :
      (unit -> ('a lin, unit, unit data) monad) ->
      ('a lin, unit, unit data) monad
  end
end
...
module type GEN_LIN = sig
  ...
  val gen_with_kinds : kind list -> 'a global -> ('b, 'b, 'a tup lin) monad

  val gen_with_kinds_mult :
    (kind * int) list -> 'a global -> ('b, 'b, 'a tup lin) monad

  val degen :
    (([ `cons of close one * 'a ] as 'a) tup lin, unit, unit data) monad
  ...
  val raw_gen_with_kinds : kind list -> 'a global -> 'a tup
  val raw_gen_with_kinds_mult : (kind * int) list -> 'a global -> 'a tup
