...
let lin_op (type b) (module D : State.StateOp with type a = b) :
    b Lin.lin State.op =
  let module M = struct
...
let gen_op (type b) (module D : State.StateOp with type a = b) :
    b Lin.gen State.op =
  let module M = struct
...
let unit =
  ...
  @@ Lazy.from_val
       State.
         { st = Lin.declare_unlimited (); st_ops = gen_op (module State.Unit) }
