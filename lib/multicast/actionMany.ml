...
module Make (DynChan : S.CHANNEL) = struct
  module ActionOut = ActionOut.Make (DynChan)
  module ActionInp = ActionInp.Make (DynChan)

  let get_many ss =
    List.map (fun s -> Lin.fresh @@ PowState.do_determinise s) ss
  ...
  let make_many ~count (names : 'v DynChan.endpoint list)
      (ss : 's many LinState.t)
      (f : 'v DynChan.endpoint -> 's LinState.t -> 'a LinState.t) :
      'a many LinState.t =
    let ss =
      lazy
        (let ss =
           Lin.fresh
           @@ PowState.do_determinise ss (* FIXME this needs context *)
         in
         ss)
    in
    let ss =
