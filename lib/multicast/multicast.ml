...
module Make (DynChan : S.CHANNEL) = struct
  module ActionScatter = ActionScatter.Make (DynChan)
  module ActionGather = ActionGather.Make (DynChan)
  module ActionMany0 = ActionMany
  module ActionMany = ActionMany0.Make (DynChan)
  ...
  type 's many = 's ActionMany0.many
  ...
  let get_chan (t : chan_table) (key : string * string) =
    ...
    | None ->
        ...
        let ch =
          List.init (from_count * to_count) (fun _i -> DynChan.make ())
        in
  ...
  let ( @@==> ) ra rb g env =
    ...
    let g =
      seq_put rb.role_index g
        (ActionGather.make_gather_val ra.role_label keys b)
    in
    let a = seq_get ra.role_index g in
    let g =
      seq_put ra.role_index g
