...
and 'a node = { mutable next : 'a node; mutable data : 'a option }
...
let rec next_rec ~f t =
  ...
  | None ->
      ...
      Lwt.protected t.push_signal >>= fun () -> next_rec ~f t
  ...
  | Some x -> (
      ...
      | Some x ->
          ...
          t.node <- t.node.next;
          Lwt.return x
      | None -> failwith "lwt_stream_opt: TODO")

(*TODO*)
(* next_rec ~f t *)

let receive t = next_rec ~f:(fun x -> Some x) t
let receive_wrap ~f t = next_rec ~f t
