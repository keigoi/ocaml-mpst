...
module Make (DynChan : S.CHANNEL) = struct
  ...
  let ( --> ) ra rb lab g env =
    ...
    let g =
      seq_put rb.role_index g
        (ActionInp.make_branch ra.role_label lab.var key b)
    in
    let a = seq_get ra.role_index g in
    let g =
      seq_put ra.role_index g
        (ActionOut.make_select rb.role_label lab.obj key a)
    in
    g
  ...
  let ( ==> ) ra rb g env =
    ...
    let a = seq_get ra.role_index g in
    let g = seq_put ra.role_index g (ActionOut.make_out rb.role_label key a) in
    g
