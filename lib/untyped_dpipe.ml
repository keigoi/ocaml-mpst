...
let close_dpipe dp =
  IO.bind (IO.close_in dp.me.inp) (fun [@inline] () ->
      IO.bind (IO.close_out dp.me.out) (fun [@inline] () -> IO.return_unit))
...
struct
  ...
  let out_dpipe ch label : 'v U.out =
   fun v ->
    let tag = Untyped.make_tag label.var in
    IO.bind
      (IO.output_value ch.me.out (tag, Obj.repr v))
      (fun () -> IO.flush ch.me.out)

  let wrap var (v, cont) = var (Obj.obj v, cont)
  ...
  let[@inline] wrap_list var (vs, cont) = var (List.map Obj.obj vs, cont)

  let[@inline] inplist_dpipe chs label cont : 'var U.inplist =
    let tag = Untyped.make_tag label.var in
    ( List.map (fun [@inline] ch () -> IO.input_value ch.me.inp) chs,
