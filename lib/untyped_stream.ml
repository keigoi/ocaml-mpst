...
struct
  ...
  let out_untyped ch label : 'v U.out =
   fun v ->
  ...
  let inp_untyped ch label cont : 'var U.inp =
    let tag = Untyped.make_tag label.var in
    ( (fun () -> Name.receive (snd ch.me)),
      [ (tag, U.Wrapper (cont, wrap label.var)) ] )

  let wrap_list var (vs, cont) = var (List.map Obj.obj vs, cont)

  let inplist_untyped chs label cont : 'var U.inplist =
    let tag = Untyped.make_tag label.var in
    ( List.map (fun ch () -> Name.receive (snd ch.me)) chs,
