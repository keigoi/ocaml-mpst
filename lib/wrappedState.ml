...
type 'var wrapped_head =
  | WrappedHead :
      ('var, 's) Types.constr * unit Name.t * 's state_id * 's head
      -> 'var wrapped_head

type 'var wrapped_head_lazy =
  | WrappedHeadLazy :
      ('var, 's) Types.constr * unit Name.t * ('s state_id * 's head list) lazy_t
      -> 'var wrapped_head_lazy
...
let make var n st =
  ref
  @@ WrappedNondet [ WrappedHeadLazy (var, n, lazy (State.epsilon_closure st)) ]

let rec classify op ws =
  match ws with
  | w1 :: ws ->
      let w, ws =
        List.fold_left
...
let concat_heads_if_constrs_are_same :
      'a.
      'a wrapped_head_lazy ->
      'a wrapped_head_lazy ->
      'a wrapped_head_lazy option =
  let do_classify newid (var1, n1, _id1, hs1) (var2, n2, _id2, hs2) =
    let vs2 = List.map (fun x -> x.head) hs2 in
    match List.filter_map (Types.cast_if_constrs_are_same var1 var2) vs2 with
    | _ :: _ as vs2 ->
    ...
    | [] -> None
  in
  fun (WrappedHeadLazy (var1, n1, (lazy (id1, hs1))))
      (WrappedHeadLazy (var2, n2, (lazy (id2, hs2)))) ->
...
let wrapped_heads : 'a wrapped_nondet -> 'a wrapped_head_lazy list =
 fun ws ->
  match ws with
  | WrappedDeterminised ws ->
      List.map
        (fun (WrappedHead (var, n, id, h)) ->
          WrappedHeadLazy (var, n, Lazy.from_val (id, [ h ])))
        ws
  | WrappedNondet ws -> ws

let merge wl wr =
  let wl' = wrapped_heads !wl and wr' = wrapped_heads !wr in
  ref (WrappedNondet (classify_wrapped_names (wl' @ wr')))

let determinise ~cache r =
  match !r with
  | WrappedNondet ws ->
      let ws =
        List.map
          (fun (WrappedHeadLazy (var, n, (lazy (id, hds)))) ->
            let hd = State.determinise_heads ~cache id hds in
...
let make_event_from_determinised_ ws =
  ...
  | _ ->
      failwith
        "determinise_wrapped: not determinised -- possible bug in \
         determinisation?"
