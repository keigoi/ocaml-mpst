module IO = struct
  ...
  let input_value ch = Lwt_io.read_value ch
  let flush ch = Lwt_io.flush ch
...
module Thread : sig
  ...
  let create f x =
    let t, u = Lwt.wait () in
    Lwt.async (fun () -> Lwt.map (fun _ -> Lwt.wakeup_later u ()) (f x));
    t
...
module Mutex : sig
  ...
  let try_lock t =
    if Lwt_mutex.is_locked t then Lwt.return false
    else
