...
let rec lens_get : type x y xs ys. (x, y, xs, ys) lens -> xs -> x =
 fun [@inline] [@specialise] l xs ->
  match (l, xs) with
  | Zero, `cons (hd, _) -> hd
  | Succ l, `cons (_, tl) -> lens_get l tl
  | Other (get, _), xs -> get xs

let rec lens_put : type x y xs ys. (x, y, xs, ys) lens -> xs -> y -> ys =
 fun [@inline] [@specialise] l xs b ->
  match (l, xs) with
...
let[@inline] return_lin v =
  { __m = (fun [@inline] pre -> IO.return (pre, { __lin = v })) }
 ...
 fun [@inline] m l ->
  {
    __m =
      (fun [@inline] pre ->
        IO.bind
          (m.__m (lens_get l pre))
          (fun [@inline] (q, a) -> IO.return (lens_put l pre q, a)));
  }
...
let get_lin :
      'a 'pre 'post.
      ('a lin, unit, 'pre, 'post) lens -> ('pre, 'post, 'a lin) monad =
 fun [@inline] l ->
  { __m = (fun [@inline] pre -> IO.return (lens_put l pre (), lens_get l pre)) }

let put_lin :
      'a 'mid 'post 'pre.
      (unit, 'a lin, 'mid, 'post) lens ->
      ('pre, 'mid, 'a lin) monad ->
...
let put_linval :
      'a 'pre 'post.
      (unit, 'a lin, 'pre, 'post) lens -> 'a -> ('pre, 'post, unit data) monad =
 fun [@inline] l v ->
  {
    __m =
      (fun [@inline] pre ->
...
let[@inline] lift m =
  {
    __m =
      (fun [@inline] p ->
        IO.bind m (fun [@inline] v -> IO.return (p, { data = v })));
  }
...
let[@inline] run_ f =
  IO.bind ((f ()).__m all_empty) (fun [@inline] (_, { data = x }) ->
      IO.return x)
...
module Syntax = struct
  ...
  let[@inline] bind_raw { __m = m } f =
    {
      __m =
        (fun [@inline] pre ->
          IO.bind (m pre) (fun [@inline] (mid, x) -> (f x).__m mid));
    }
  ...
  module Internal = struct
    ...
    let _mkbind : 'f. 'f -> 'f bind = fun [@inline] f -> f

    (* external _mkbind : 'a -> 'a = "%identity" *)
    let _run :
          'pre 'post 'a. ('pre, 'post, 'a) monad -> 'pre -> ('post * 'a) IO.io =
    ...
    let _peek :
          'pre 'post 'a.
          ('pre -> ('pre, 'post, 'a) monad) -> ('pre, 'post, 'a) monad =
     fun [@inline] f -> { __m = (fun [@inline] pre -> (f pre).__m pre) }
     ...
     fun [@inline] f l ->
      {
        __m =
          (fun [@inline] pre ->
            IO.return
              ( lens_put l pre @@ _lin (f (_unlin @@ lens_get l pre)),
                { data = () } ));
      }

    let _modify :
          'pre 'mid 'post 'a.
          ('pre -> 'mid) -> ('mid, 'post, 'a) monad -> ('pre, 'post, 'a) monad =
     fun f m -> { __m = (fun [@inline] pre -> m.__m (f pre)) }
  end
end
