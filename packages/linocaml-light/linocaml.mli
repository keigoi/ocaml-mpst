...
val _1 :
  ( 'a,
    'b,
    [ `cons of 'x1 * [ `cons of 'a * 'xs ] ],
    [ `cons of 'x1 * [ `cons of 'b * 'xs ] ] )
  lens

val _2 :
  ( 'a,
    'b,
    [ `cons of 'x1 * [ `cons of 'x2 * [ `cons of 'a * 'xs ] ] ],
    [ `cons of 'x1 * [ `cons of 'x2 * [ `cons of 'b * 'xs ] ] ] )
  lens
...
val ( @> ) :
  ('p, 'q, 'a) monad -> ('p, 'q, 'pre, 'post) lens -> ('pre, 'post, 'a) monad

val ( <@ ) :
  ('p, 'q, 'pre, 'post) lens -> ('p, 'q, 'a) monad -> ('pre, 'post, 'a) monad
...
val thread_create :
  ...
  'd ->
  ('b, 'c, Concur_shims.Thread.t data) monad

val lift : 'a Concur_shims.IO.io -> ('p, 'p, 'a data) monad

val run :
  ('a -> (all_empty, all_empty, 'b data) monad) -> 'a -> 'b Concur_shims.IO.io

val run_ :
  (unit -> (all_empty, all_empty, 'b data) monad) -> 'b Concur_shims.IO.io
...
module Syntax : sig
  ...
  val lens_put' : ('x, 'y lin, 'xs, 'ys) lens -> 'y -> 'xs -> 'ys

  module Internal : sig
    val _mkbind : 'f -> 'f bind

    val _run :
      ('pre, 'post, 'a) monad -> 'pre -> ('post * 'a) Concur_shims.IO.io

    val _all_empty : all_empty
