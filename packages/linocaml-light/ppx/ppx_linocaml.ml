...
let evar ?loc ?attrs ~quals id = Exp.ident ?loc ?attrs (lid ?loc ~quals id)

let record ?loc ?attrs ?over ?(quals = []) l =
  Exp.record ?loc ?attrs
    (List.map (fun (s, e) -> (lid ~loc:e.pexp_loc ~quals s, e)) l)
    over

let constr ?loc ?attrs ~quals s args =
  Exp.construct ?loc ?attrs (lid ?loc ~quals s) (may_tuple ?loc Exp.tuple args)

let precord ?loc ?attrs ?(closed = Open) ~quals l =
  Pat.record ?loc ?attrs
    (List.map (fun (s, e) -> (lid ~loc:e.ppat_loc ~quals s, e)) l)
    closed

let tconstr ?loc ?attrs ~quals c l =
  Typ.constr ?loc ?attrs (lid ?loc ~quals c) l

let unit ?loc ?attrs () = constr ?loc ?attrs "()" []

let pconstr ?loc ?attrs s args =
  Pat.construct ?loc ?attrs (lid ?loc ~quals:[] s)
    (may_tuple ?loc Pat.tuple args)
...
let monad_bind_data () = longident ~quals:[ !root_module ] "bind_data"
let monad_bind_lin () = longident ~quals:[ !root_module ] "bind_lin"
let monad_return_lin () = longident ~quals:[ !root_module ] "return_lin"
let get_lin () = longident ~quals:[ !root_module ] "get_lin"
let put_linval () = longident ~quals:[ !root_module ] "put_linval"
let lens_put' () = longident ~quals:[ !root_module ] "lens_put'"
let mkbind () = longident ~quals:[ !root_module; "Internal" ] "_mkbind"
let modify () = longident ~quals:[ !root_module; "Internal" ] "_modify"
  ...
  let insert_expr (linvar, newvar) =
    app (modify ())
      [
        app (lens_put' ())
          [
            Exp.ident ~loc:linvar.loc linvar;
            evar ~loc:linvar.loc ~quals:[] newvar;
          ];
      ]
  in
  List.fold_right (fun e expr -> app (insert_expr e) [ expr ]) es expr
...
let convert_pattern (p : pattern) :
    pattern * (Longident.t Location.loc * string) list =
  let lin_vars = ref [] in
  ...
  let rec traverse ({ ppat_desc; _ } as patouter) =
    ...
    | Ppat_type lidloc -> replace_linpat lidloc (* #tconst *)
    | Ppat_any -> wrap_datapat patouter (* _ *)
    | Ppat_var _ -> wrap_datapat patouter (* x *)
    | Ppat_constant _ (* 1, 'a', "true", 1.0, 1l, 1L, 1n *)
    | Ppat_interval (_, _) (* 'a'..'z' *)
    | Ppat_construct (_, None) (* constructor without payloads*)
    | Ppat_variant (_, None) ->
        (* polymorphic variant constructor without payloads*)
        patouter
    | Ppat_tuple pats ->
        { patouter with ppat_desc = Ppat_tuple (List.map traverse pats) }
        (* (P1, ..., Pn)

           Invariant: n >= 2
        *)
    | Ppat_construct (lidloc, Some pat) ->
        {
          patouter with
          ppat_desc = Ppat_construct (lidloc, Some (traverse pat));
        }
        (* C                None
           C P              Some P
           C (P1, ..., Pn)  Some (Ppat_tuple [P1; ...; Pn])
        *)
    | Ppat_variant (lab, Some pat) ->
        { patouter with ppat_desc = Ppat_variant (lab, Some (traverse pat)) }
        (* `A             (None)
           `A P           (Some P)
        *)
    | Ppat_record ([ ({ txt = Lident "data"; _ }, _) ], Closed) as p ->
        { patouter with ppat_desc = p }
    | Ppat_record (recpats, Closed) ->
        {
          patouter with
          ppat_desc =
            Ppat_record
              ( List.map (fun (field, pat) -> (field, traverse pat)) recpats,
                Closed );
        }
        ...
        *)
    | Ppat_array pats ->
        { patouter with ppat_desc = Ppat_array (List.map traverse pats) }
        (* [| P1; ...; Pn |] *)
    | Ppat_constraint (pat, typ) ->
        { patouter with ppat_desc = Ppat_constraint (traverse pat, typ) }
        (* (P : T) *)
    | Ppat_lazy pat -> { patouter with ppat_desc = Ppat_lazy (traverse pat) }
    | Ppat_alias (_, tvarloc) ->
        error tvarloc.loc "as-pattern is forbidden at %lin match"
        (* TODO relax this *)
...
let lin_pattern oldpat : pattern * (Longident.t Location.loc * string) list =
  let wrap ({ ppat_loc; _ } as oldpat) =
    ...
    let newpat =
      if is_linpat oldpat then newpat (* not to duplicate Lin pattern *)
      else precord ~loc:ppat_loc ~quals:[ "Linocaml" ] [ ("__lin", newpat) ]
    in
    (newpat, lin_vars)
  in
...
let rec linval ({ pexp_desc; pexp_loc; _ (*pexp_loc_stack*) } as outer) =
  ...
  | Pexp_tuple exprs ->
      ...
      ( { outer with pexp_desc = Pexp_tuple exprs; pexp_loc },
        List.concat bindings )
  | Pexp_record ([ ({ txt = Lident "data"; _ }, _) ], None) -> (outer, [])
  ...
  | Pexp_apply (expr, exprs) -> (
      ...
      let exprs, bindings =
        ...
        @@ List.map
             ...
             exprs
      in
      match binding @ List.concat bindings with
      | [] -> ({ outer with pexp_desc = Pexp_apply (expr, exprs) }, [])
      | _ ->
          error pexp_loc
            "function call inside %linval cannot contain slot references (!! \
             slotname)")
  | Pexp_object
      ({ pcstr_self = { ppat_desc = Ppat_any; _ }; pcstr_fields = fields } as o)
    ->
      let new_fields, bindings =
        ...
        @@ List.map
             (function
               | {
                   pcf_desc = Pcf_method (name, Public, Cfk_concrete (fl, expr));
                   _;
                 } as f ->
                   ...
                   ( {
                       f with
                       pcf_desc =
                         Pcf_method (name, Public, Cfk_concrete (fl, new_expr));
                     },
                     binding )
               | _ -> error pexp_loc "object can only contain public method")
             fields
      in
      ( {
          outer with
          pexp_desc = Pexp_object { o with pcstr_fields = new_fields };
        },
        List.concat bindings )
  | Pexp_object _ -> failwith "object in linval can't refer to itself"
  | Pexp_poly (expr, None) ->
      let expr, binding = linval expr in
      ({ outer with pexp_desc = Pexp_poly (expr, None) }, binding)
  | Pexp_poly (_, _) -> failwith "object method can not have type ascription"
  | Pexp_let (_, _, _)
  | Pexp_function _
  | Pexp_fun (_, _, _, _)
  | Pexp_match (_, _)
  ...
  | Pexp_assert _
  | Pexp_newtype (_, _)
  | Pexp_pack _ | Pexp_extension _ | Pexp_unreachable | Pexp_letexception _
  | Pexp_letop _ ->
...
let expression_mapper id mapper exp attrs =
  ...
  let process_inner expr = mapper.Ast_mapper.expr mapper expr in
  ...
  | "lin", Pexp_let (Nonrecursive, vbls, expr) ->
      ...
      let make_bind { pvb_pat; pvb_expr; pvb_loc; _ } expr =
        app ~loc:pexp_loc ~attrs:pexp_attributes (monad_bind_lin ())
          [
            pvb_expr;
            app ~loc:pvb_loc (mkbind ()) [ lam ~loc:pvb_loc pvb_pat expr ];
          ]
      in
  ...
  | "lin", Pexp_match (matched, cases) ->
      ...
      let new_expr =
        app ~loc:pexp_loc ~attrs:pexp_attributes (monad_bind_lin ())
          [
            matched;
            app ~loc:pexp_loc (mkbind ())
              [ Exp.function_ ~loc:pexp_loc new_cases ];
          ]
      in
      Some (process_inner new_expr)
  | "lin", Pexp_function cases ->
      ...
      let new_expr =
        app ~loc:pexp_loc ~attrs:pexp_attributes (mkbind ())
          [
            {
  ...
  | "lin", Pexp_fun (Nolabel, None, pat, expr) ->
      ...
      let new_expr =
        app ~loc:pexp_loc ~attrs:pexp_attributes (mkbind ())
          [
            {
              pexp_desc = Pexp_fun (Nolabel, None, newpat, newexpr);
              pexp_loc;
              pexp_attributes;
              pexp_loc_stack;
            };
  ...
  | "lin", _ ->
      error pexp_loc
        "Invalid content for extension %lin; it must be \"let%lin slotname = \
         ..\" OR \"match%lin slotname with ..\""
  | "linret", expr ->
      let new_exp, bindings =
        linval { pexp_desc = expr; pexp_loc; pexp_attributes; pexp_loc_stack }
      in
  ...
  let expr mapper outer =
    ...
    | {
     pexp_desc =
       Pexp_extension
         ( { txt = id; _ },
           PStr [ { pstr_desc = Pstr_eval (inner, inner_attrs); _ } ] );
     pexp_attributes = outer_attrs;
     _;
    } -> (
        match expression_mapper id mapper inner (inner_attrs @ outer_attrs) with
        | Some exp -> exp
...
let () =
  Driver.register ~name:"ppx_linocaml" Versions.ocaml_408
